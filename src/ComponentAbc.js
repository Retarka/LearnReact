...
export default class ComponentAbc extends Component {
  ...
  ClickToChangeLetters(event) {
    ...
    this.state.items.forEach(
      (element) => (valid = valid && element !== event.target.value)
    );
  ...
  // Удаляет весь массив с символами
  ...
  // Удаляет результат по одной срочке (нет указанного индекса)
  ...
  render() {
    return (
      <div>
        <h1>Alphabet</h1>
        <div>{/* Место для компонента */}</div>
        <form onSubmit={this.ClickToSubmitLetters} className={"form_submit"}>
          <label>Write the letter </label>
          <input
            value={this.state.input}
            maxLength={1}
            onChange={this.ClickToChangeLetters}
            id="input-name"
            placeholder="abc"
          ></input>
          <button onClick={this.ClickToSubmitLetters} type={"submit"}>
            CLICK TO SUBMIT +
          </button>
          <ul>
            {this.state.items.map((letter, index) => (
              <li>
                ...
                <button
                  ...
                  onClick={(event) => {
                    this.ClickOnDeleteBtn(event, index);
                  }}
                >
                  x
                </button>
        ...
        <form className={"form_delete"}>
          <button type={"button"} onClick={this.ClickToDeleteLetter}>
            DELETE BY ONE
          </button>
          <button type={"button"} onClick={this.ClickToDeleteLetters}>
            CLICK TO DELETE ALL
          </button>
        </form>
      </div>
